...
def filter_search_volume(file):
    ...
    #set na if there is -
    dataset['Search Volume'] = pd.to_numeric(dataset['Search Volume'], errors='coerce')
    #remove - or na data
    dataset.dropna(subset=['Search Volume'], inplace=True)

    filtered_dataframe = dataset[dataset['Search Volume'].astype(int) >= min_search_volume]
...
def keywords_to_txt(file, filename:str=None):
    ...
    keywords = dataframe['Keyword'].tolist()
    with open(f'{filename}.txt', 'w') as file:
        file.write(','.join(keywords))

basedir = "C:/Users/spart/Desktop/Projects ongoing/Cresteem website/kw/"
exdirs = ["competitor"]
